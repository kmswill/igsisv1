		   ...
		   ?> 
				...
				<table class="table table-condensed">
					...
					while($descricao = mysqli_fetch_array($query_busca)){
						...
						<input type ='submit' class='btn btn-theme btn-sm btn-block' value='anexos pessoa'></td></form>"	; //botão de edição
						echo "
						<td  class='list_description'>
						<form method='POST' action='?perfil=contratados&p=arq_pedidos&id_ped=".$descricao['idPedidoContratacao']."'>
						<input type='hidden' name='idPessoa' value='".$descricao['idPessoa']."'>
						<input type='hidden' name='tipoPessoa' value='".$descricao['tipoPessoa']."'>

						<input type ='submit' class='btn btn-theme btn-sm btn-block' value='anexos pedido'></td></form>"	; //botão de edição
			  ...
			  <div class="row">
				  <div class="col-md-offset-2 col-md-8">
					<div class="section-heading">
                     ...
                     <p><?php if(isset($mensagem)){echo $mensagem;} ?></p>
<p>Nesta página, você envia documentos digitalizados de pessoas físicas e jurídicas. Não envie arquivos que dizem respeito ao pedido contratação como documentos de executante, O tamanho máximo do arquivo deve ser 60MB.</p>
...
<br />
<div class = "center">
<form method="POST" action="?perfil=contratados&p=arq_pedidos&id_ped=<?php echo $_GET['id_ped']; ?>" enctype="multipart/form-data">
<table>
<tr>
<td width="50%"><td>
</tr>
