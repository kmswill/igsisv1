...
function enviarEmail($conteudo_email, $instituicao, $subject, $idEvento, $num_pedidos ){ 
	...
	try {
	  ...
	  //criar laço com todos os interessados
	  	$con = bancoMysqli();
		if($num_pedidos == -1){
		$sql_user = "SELECT DISTINCT ig_usuario.nomeCompleto, ig_usuario.email FROM ig_usuario, ig_papelusuario WHERE ig_papelusuario.idPapelUsuario = ig_usuario.ig_papelusuario_idPapelUsuario AND ig_usuario.receberNotificacao = '1' AND
		(
		ig_papelusuario.admin = '1' OR
		ig_papelusuario.administrador = '1' 
		)
		...
		$sql_user = "SELECT DISTINCT ig_usuario.nomeCompleto, ig_usuario.email FROM ig_usuario, ig_papelusuario WHERE ig_usuario.idInstituicao = '$instituicao' AND ig_papelusuario.idPapelUsuario = ig_usuario.ig_papelusuario_idPapelUsuario AND ig_usuario.receberNotificacao = '1' AND
		(
		ig_papelusuario.admin = '1' OR
...
function recuperaDuracao($id){ //retorna duração
...
function listaFilmes($idEvento){ //lista ocorrencias de determinado evento
...
function listaOcorrenciasCinema($idCinema){ //lista ocorrencias de determinado evento
...
function listaOcorrenciasFilmes($idCinema){ //lista ocorrencias de determinado filme
	$sql = "SELECT * FROM ig_ocorrencia WHERE idCinema = '$idCinema' AND publicado = 1 ORDER BY dataInicio";
	$con = bancoMysqli();
	$query = mysqli_query($con,$sql);
	while($campo = mysqli_fetch_array($query)){
...
function listaServicosInternos($idEvento){
	...
	if($interno){
		if(($interno['carros'] != '') AND ($interno['carros'] != NULL)){ echo "Carros: ".nl2br($interno['carros'])."<br />";}	
		if(($interno['equipe'] != '') AND ($interno['equipe'] != NULL)){ echo "Equipe: ".nl2br($interno['equipe'])."<br />";}	
		if(($interno['infraestrutura'] != '') AND ($interno['infraestrutura'] != NULL)){ echo "Infraestrutura: ".nl2br($interno['infraestrutura'])."<br />";}	
		if($interno['registroAudio'] == 1){ echo "Registro de Áudio: Sim <br />";}	
		if($interno['registroFotografia'] == 1){ echo "Registro de Fotografia: Sim <br />";}	
		if($interno['registroVideo'] == 1){ echo "Registro de Vídeo: Sim <br />";}	
	} else {
		echo "Não há dados.";	
	}
	...
	if($evento['ig_tipo_evento_idTipoEvento'] == '2'){
		...
		if($artes){
			...
			if($artes['painel'] == 1){ echo "Confecção de Painel: Sim <br />";}	
			if($artes['legendas'] == 1){ echo "Confecção de Legendas: Sim <br />";}	
			if($artes['identidade'] == 1){ echo "Criação de Identidade Visual: Sim <br />";}	
			if($artes['suporte'] == 1){ echo "Pedido de suporte de comunicação: Sim <br />";}	
		}
	}
}
...
function listaServicosExternos($idEvento){
	...
	if($externo){
	if(($externo['legenda'] != '') AND ($externo['legenda'] != NULL)){ echo "Legenda / legendagem: ".$externo['legenda']."<br />";}
	if(($externo['traducao'] != '') AND ($externo['traducao'] != NULL)){ echo "Tradução: ".$externo['traducao']."<br />";}
	if(($externo['seguro'] != '') AND ($externo['seguro'] != NULL)){ echo "Seguro: ".$externo['seguro']."<br />";}
	if(($externo['transporte'] != '') AND ($externo['transporte'] != NULL)){ echo "Transporte: ".$externo['transporte']."<br />";}
	if(($externo['montagem'] != '') AND ($externo['montagem'] != NULL)){ echo "Montagem fina: ".$externo['montagem']."<br />";}
	if(($externo['passagens'] != '') AND ($externo['passagens'] != NULL)){ echo "Passagem aérea: ".$externo['passagens']."<br />";}
	if(($externo['itinerario'] != '') AND ($externo['itinerario'] != NULL)){ echo "Descrição da passagem: ".$externo['itinerario']."<br />";}
	if(($externo['hospedagem'] != '') AND ($externo['hospedagem'] != NULL)){ echo "Hospedagem: ".$externo['hospedagem']."<br />";}
	if(($externo['locacao'] != '') AND ($externo['locacao'] != NULL)){ echo "Locação de equipamentos: ".$externo['locacao']."<br />";}
	if(($externo['bilhetagem'] != '') AND ($externo['bilhetagem'] != NULL)){ echo "<br />Bilhetagem: <br /> ".nl2br($externo['bilhetagem'])."<br />";}
	}else{
		echo "Não há dados.";	
	}
}

function retornaDatas($id){ //retorna o período
	$con = bancoMysqli();
	$evento = recuperaDados("ig_evento",$id,"idEvento");
	
	
	if($evento['ig_tipo_evento_idTipoEvento'] != 1){ //Não é Mostra de Cinema
	$sql_anterior = "SELECT * FROM ig_ocorrencia WHERE idEvento = '$id' AND publicado = '1' ORDER BY dataInicio ASC LIMIT 0,1"; //a data inicial mais antecedente
	$query_anterior = mysqli_query($con,$sql_anterior);
	...
	}else{ // é Mostra de Cinema
	$sql_anterior = "SELECT * FROM ig_ocorrencia WHERE idEvento = '$id' AND publicado = '1' AND idTipoOcorrencia = '5' ORDER BY dataInicio ASC LIMIT 0,1"; //a data inicial mais antecedente
	$query_anterior = mysqli_query($con,$sql_anterior);
	$data_i = mysqli_fetch_array($query_anterior);
	$data_inicio = $data_i['dataInicio'];
	
	$sql_posterior01 = "SELECT * FROM ig_ocorrencia WHERE idEvento = '$id' AND publicado = '1' AND idTipoOcorrencia = '5'  ORDER BY dataFinal DESC LIMIT 0,1"; //quando existe data final
	$sql_posterior02 = "SELECT * FROM ig_ocorrencia WHERE idEvento = '$id' AND publicado = '1' AND idTipoOcorrencia = '5'  ORDER BY dataInicio DESC LIMIT 0,1"; //quando há muitas datas únicas
	...
	if(($num > 0) AND ($data['dataFinal'] != '0000-00-00')){ //verifica se é temporada
		$dataFinal01 = $data['dataFinal'];	
	}else{
...
function resumoOcorrenciasDoc($idEvento){
	...
	function primeiraData($idEvento){
		$con = bancoMysqli();
		$sql_primeiro = "SELECT dataInicio FROM ig_ocorrencia WHERE idEvento = '$idEvento' ORDER BY dataInicio DESC LIMIT 0,1";
...
function listaOcorrenciasInstituicao($idInstituicao,$ordem){
	...
	while($sala = mysqli_fetch_array($query_sala)){
		...
		while($ocorrencia = mysqli_fetch_array($query_ocorrencia)){
			...
			if($evento['publicado'] == 1 AND $evento['dataEnvio'] != NULL ){ // somente os publicados e enviados
				...
				if($num == 0){
					$sql_temp = "INSERT INTO `temp_data` ( `idEvento`, `dataInicio`, `dataEnvio`) VALUES ( '$idEvento', '$dataInicio', '$dataEnvio')"; //insere numa tabela temporária
...
function gradeFilmes($idEvento){
	$con = bancoMysqli();
	$sql = "SELECT * FROM ig_ocorrencia WHERE idEvento = '$idEvento' AND publicado = '1' ORDER BY dataInicio, horaInicio ASC";
	$query = mysqli_query($con,$sql);
	$data_antiga = "";
...
function listaSubEventosCom($idEvento){ //lista ocorrencias de determinado evento
	$sql = "SELECT * FROM ig_sub_evento WHERE ig_evento_idEvento = '$idEvento' AND publicado = '1' ORDER BY idSubEvento DESC";
	$con = bancoMysqli();
	$query = mysqli_query($con,$sql);
...
function listaOcorrenciasContrato($idEvento){ //lista ocorrencias de determinado evento
	$sql = "SELECT * FROM ig_ocorrencia WHERE idEvento = '$idEvento' AND publicado = 1 AND idTipoOcorrencia NOT LIKE '5' ORDER BY dataInicio";
	$con = bancoMysqli();
	$query = mysqli_query($con,$sql);
	$evento = recuperaDados("ig_evento",$idEvento,"idEvento");
